        ...
        <vue-good-table
            ...
            <template slot="table-row" slot-scope="props">
                <span v-if="props.column.field === 'avatar'">
                    <div v-html='getAvatarRow(props.row)'></div>
                </span>
                <span v-if="props.column.field === 'name'">
                    <div v-html='getNameRow(props.row)'></div>
                </span>
            </template>
        </vue-good-table>
...
<script>
    export default {
        ...
        mounted() {
            this.prepareComponent(this.htmldir)
        },
        ...
        methods: {
             ...
             */
            prepareComponent() {
                axios.get('/people/list'+window.location.search)
                    .then(response => {
            ...
            getAvatarRow(row) {
                if (row.avatar_type === 'url' || row.avatar_type === 'avatar') {
                    return '<img src="'+row.avatar+'" width="43">';
                } else if (row.avatar_type === 'initial') {
                    return '<div class="avatar one-letter" style="background-color: '+row.avatar_color+'">'+row.avatar+'</div>';
                }
                return '<div class="avatar" style="background-color: '+row.avatar_color+'">'+row.avatar+'</div>';
            },

            getNameRow(row) {
                let nameRow = '<span>'+row.name+'</span>'

                let dir = (this.htmldir === 'ltr') ? 'ml3' : 'mr3'

                if (row.description) {
                    nameRow += '<span class="i '+dir+'">'+row.description+'</span>'
                }
