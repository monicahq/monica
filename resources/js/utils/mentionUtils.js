    ...
    if (contacts) {
      ...
      if (contact) {
        ...
        if (highlightedContact && contactId === highlightedContact) {
          return `<a href="${contactUrl}" class="text-blue-500 hover:underline"><strong>@${contactName}</strong></a>`;
        }
        return `<a href="${contactUrl}" class="text-blue-500 hover:underline">@${contactName}</a>`;
      }
    }
