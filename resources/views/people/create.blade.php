      ...
      <div class="col-xs-12 col-sm-6 col-sm-offset-3">
        ...
        <form action="/people" method="POST">
          ...
          <dl class="form-group {{ $errors->has('first_name') ? ' errored' : '' }}">
            ...
            <dd><input type="text" class="form-control" name="first_name" placeholder="" autofocus  value="{{ old('first_name') }}" autocomplete="off"></dd>
            @if ($errors->has('first_name'))
            <dd class="error">{{ $errors->first('first_name') }}</dd>
            @endif

            <dt><label for="last_name">{{ trans('people.people_add_lastname') }}</label></dt>
            <dd><input type="text" class="form-control" name="last_name" placeholder="" value="{{ old('last_name') }}" autocomplete="off"></dd>
