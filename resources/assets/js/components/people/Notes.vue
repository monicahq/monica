    ...
    <div>
      <div>
        <form class="bg-near-white pa2 br2 mb3">
          ...
          ></textarea>
          <a v-if="addMode" class="pointer btn btn-primary" cy-name="add-note-button" @click.prevent="store" href="">
            {{ $t('app.add') }}
          </a>
          <a v-if="addMode" class="pointer btn btn-secondary" cy-name="cancel-note-button" @click.prevent="addMode = false" href="">
            {{ $t('app.cancel') }}
          </a>
        </form>
      </div>
      ...
      <ul>
        <li v-for="note in notes" :key="note.id" class="note">
          <div v-show="!note.edit" class="ba br2 b--black-10 br--top w-100 mb2" :cy-name="'note-body-' + note.id">
            ...
            <div class="pa2 cf bt b--black-10 br--bottom f7 lh-copy">
              ...
              <div class="fl w-50 tr">
                <a class="pointer" :cy-name="'edit-note-button-' + note.id" @click.prevent="toggleEditMode(note)" href="">
                  {{ $t('app.edit') }}
                </a>
                |
                <a class="pointer" :cy-name="'delete-note-button-' + note.id" @click.prevent="showDelete(note)" href="">
                  {{ $t('app.delete') }}
                </a>
              </div>
            </div>
          ...
          <form v-show="note.edit" class="bg-near-white pa2 br2 mt3 mb3">
            ...
            <a class="pointer btn btn-primary" :cy-name="'edit-mode-note-button-' + note.id" @click.prevent="update(note)" href="">
              {{ $t('app.update') }}
            </a>
          </form>
        </li>
