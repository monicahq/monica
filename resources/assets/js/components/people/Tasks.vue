...
<template>
  <div>
    ...
    <div v-bind:class="[editMode ? 'bg-washed-yellow b--yellow ba pa2' : '']">
      ...
      <ul>
        <li v-for="task in inProgress(tasks)" :cy-name="'task-item-' + task.id" v-bind:key="task.id">
      ...
      <ul>
        <li v-for="task in completed(tasks)" class="f6" :cy-name="'task-item-completed-' + task.id" v-bind:key="task.id">
          <input type="checkbox" id="checkbox" v-model="task.completed" @click="toggleComplete(task)" class="mr1">
