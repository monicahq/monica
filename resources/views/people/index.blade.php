...
@section('content')
  <div class="people-list {{ direction() }}">
    {{ csrf_field() }}
              ...
              <div class="col-xs-12">
                  @if (! is_null($tags))
                      <p class="clear-filter">
                        ...
                        @foreach ($tags as $tag)
                            <span class="pretty-tag {{ direction() }}">
                            {{ $tag->name }}
                            </span>
                        @endforeach
                        <a class="{{ direction() }}" href="{{ route('people.index') }}">{{ trans('people.people_list_clear_filter') }}</a>
                      </p>
                  @endif
                  @if ($tagLess)
                      <p class="clear-filter">
          ...
          <div class="row">
            ...
            <div class="col-xs-12 col-md-9 mb4">
              ...
              <ul class="list">
                ...
                <li class="people-list-item bg-white pointer" @click="window.location.href='{{ route('people.show', $contact) }}'">
                  ...
                  <span class="people-list-item-information {{ direction() }}">
            ...
            <div class="col-xs-12 col-md-3 sidebar">
              ...
              @foreach ($userTags as $dbtag)
                ...
                <li>
                    <span class="pretty-tag {{ direction() }}"><a href="{{ route('people.index') }}?{{$url}}tag{{$tagCount}}={{ $dbtag->name_slug }}">{{ $dbtag->name }}</a></span>
                    <span class="number-contacts-per-tag {{ direction() }}">{{ trans_choice('people.people_list_contacts_per_tags', $dbtag->contacts()->count(), ['count' => $dbtag->contacts()->count()]) }}</span>
                </li>
                @endif
              @endforeach
