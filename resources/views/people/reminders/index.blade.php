  ...
  <ul class="table">
    ...
    <li class="table-row">
      ...
      <div class="table-cell list-actions">
        ...
        @if ($reminder->delible)
        <a href="{{ route('people.reminders.edit', [$contact, $reminder]) }}" style="display:inherit;padding:unset">
          <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
        </a>
        <form method="POST" style="display:inherit" action="{{ route('people.reminders.destroy', [$contact, $reminder]) }}">
          @method('DELETE')
          @csrf
          <confirm message="{{ trans('people.reminders_delete_confirmation') }}">
            <i class="fa fa-trash-o ml1" aria-hidden="true"></i>
          </confirm>
        </form>
        @endif
      </div>
