  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        {{ $activity->getSummary() }}
        <button type="button" class="close {{ direction() }}" data-dismiss="modal">
      ...
      <div class="modal-body">
              ...
              <label for="summary">{{ trans('people.activities_who_was_involved') }}</label>
                  <ul class="contacts {{ direction() }}">
                      <ul class="contacts-list">
                          @foreach ($activity->contacts as $contact)
                              <li class="pretty-tag {{ direction() }}"><a href="{{ route('people.show', $contact) }}">{{ $contact->first_name }} {{ $contact->last_name }}</a></li>
                          @endforeach
                      </ul>
                  </ul>
                  <br>
