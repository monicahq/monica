...
return [
    ...
    'export_be_patient' => 'Нажмите на кнопку, чтобы начать экспорт. Обработка экспорта может занять несколько минут – пожалуйста, будьте терпеливы и не повторяйте нажатие на кнопку.',
    'export_title_sql' => 'Экспорт данных в SQL',
    'export_sql_explanation' => 'Экспорт данных в формате SQL позволяет импортировать данные в свой собственный экземпляр Monica. Это полезно только если у вас есть собственный сервер.',
    'export_sql_cta' => 'Экспорт данных в SQL',
    ...
    'delete_other_desc' => 'Ваши данные в основной базе данных будут удалены немедленно. Как описано в нашей политике конфиденциальности, мы выполняем ежедневные резервные копии, безопасно зашифрованные резервные копии базы данных и эта резервная копия хранится в течение 30 дней, после которых она полностью удалена. Мы не можем удалить конкретные данные из резервных копий, которые мы храним раньше. Все ваши данные будут полностью удалены в течение 30 дней с момента удаления учетной записи.',
