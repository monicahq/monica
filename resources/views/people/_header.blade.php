<div class="ph3 ph5-ns pv2 cf w-100 mt4 mt0-ns">
    ...
    <div class="relative center dib z-3">
      <div class="relative hide-child">
        <div class="image-header top-0 left-0">
          <img class="cover br3 bb b--gray-monica" alt="{{ $contact->initials }}" src="{{ $contact->getAvatarURL() }}" style="height: 115px; width: 115px;" v-on:error="fixAvatarDisplay" />
          <div class="hidden br3 dib white tc f1" style="padding-top: 21px; height: 115px; width: 115px; background-color: {{ $contact->default_avatar_color }}">
            {{ $contact->initials }}
          </div>
        </div>
  ...
  <div class="mw9 center dt w-100 box-shadow pa4 relative">
    ...
    <h1 class="tc mb2 mt4">
      ...
      @if ($contact->job || $contact->description)
      <span class="db f5 normal">
        @if ($contact->job)
          {{ $contact->job }}
        @endif
