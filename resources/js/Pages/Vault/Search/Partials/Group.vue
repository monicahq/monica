...
<template>
  <div>
    ...
    <ul v-if="data.length > 0" class="mb-6 rounded-lg border border-gray-200 bg-white">
      <li v-for="group in data" :key="group.id" class="item-list border-b border-gray-200 p-3 hover:bg-slate-50">
        <inertia-link :href="group.url" class="text-blue-500 hover:underline">
          {{ group.name }}
        </inertia-link>
      </li>
    </ul>
