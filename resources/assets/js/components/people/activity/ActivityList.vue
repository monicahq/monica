...
<template>
    <div>
        <div class="">
            <h3>
                ⛺️ {{ $t('people.activity_title') }}

                <span class="fr relative" style="top: -7px;">
                    <a @click="addMode = true" class="btn edit-information" v-if="addMode == false && displayUploadError == false && displayUploadProgress == false">{{ $t('people.document_list_cta') }}</a>
                    <a @click="addMode = false; displayUploadError = false; displayUploadProgress = false" class="btn edit-information" v-if="addMode || displayUploadError || displayUploadProgress">{{ $t('app.cancel') }}</a>
                </span>
            </h3>
        </div>

        <!-- EMPTY STATE -->
        <div class="ltr w-100 pt2" v-if="addMode == false && displayUploadError == false && displayUploadProgress == false && activities.length == 0">
            <div class="section-blank">
                <h3 class="mb0">{{ $t('people.document_list_blank_desc') }}</h3>
            </div>
        </div>

        <!-- FIRST STEP OF DOCUMENT UPLOAD -->
        <transition name="fade">
            <div class="ba br3 document-upload-zone mb3 pa3" v-if="addMode">
                <div class="tc">
                ...
                <div class="tc dib w-100 relative">
                    <button class="btn">{{ $t('people.document_upload_zone_cta') }}</button>
        ...
        <!-- LIST OF ACTIVITIES -->
        <div class="table">
            <div class="table-row" v-for="document in activities" :key="document.id">
                <div class="table-cell document-date f6">
                    {{ formatTime(document.created_at) }}
                </div>
                <div class="table-cell document-type tc">
                    <span class="document-type-icon">{{ document.type }}</span>
                </div>
                ...
                <div class="table-cell document-action relative">
                    <div class="document-action-wrapper pointer" @click="toggleActionsModal(document.id)">
                        <svg class="document-action-button" width="24" height="6" viewBox="0 0 24 6" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    export default {
        ...
        methods: {
            ...
            submitFile(){
                ...
                axios.post( '/people/' + this.hash + '/document/store',
                    formData,
                    {
                    headers: {
            ...
            deleteDocument(document) {
                axios.delete( '/people/' + this.hash + '/activities/' + document.id)
                    .then(response => {
                        this.activities.splice(this.activities.indexOf(document), 1);
                    })
                    .catch(error => {
                    });
            },
