      ...
      <div class="{{ Auth::user()->getFluidLayout() }}">
        <div class="row">
          <div class="col-xs-12 col-sm-6 col-sm-offset-3">
            ...
            <form method="POST" action="{{ route('people.reminders.save', ['person' => $people->id, 'reminderId' => $reminder->id]) }}">
              {{ csrf_field() }}

              <h2>{{ trans('people.reminders_add_title', ['name' => $people->getFirstName()]) }}</h2>
                    ...
                    <select class="form-control" name="reminderPredefinedTypeId">
                      @foreach ($reminderTypes as $reminderType)
                        ...
                        <option value="{{ $reminderType->id }}">
                            {{ trans($reminderType->translation_key) }} {{ $people->getFirstName() }}
                        </option>
                        @endif
                      @endforeach
                    </select>
              ...
              <div class="form-group">
                ...
                <input type="date" id="reminderNextExpectedDate" name="reminderNextExpectedDate" class="form-control"
                    value="{{ \Carbon\Carbon::now($timezone)->format('Y-m-d') }}"
                    min="{{ \Carbon\Carbon::now($timezone)->format('Y-m-d') }}"
                    max="{{ \Carbon\Carbon::now($timezone)->addYears(10)->format('Y-m-d') }}">
              ...
              <div class="form-group actions">
                ...
                <a href="{{ route('people.show', ['people' => $people->id]) }}" class="btn btn-secondary">{{ trans('app.cancel') }}</a>
              </div>
            </form>
          </div>
        </div>
