...
class CalDAVBackend extends AbstractBackend implements SyncSupport
{
     ...
     *
     * @return Collection<array-key,AbstractCalDAVBackend>
     */
    private function getBackends(): Collection
    {
        $backends = collect([
            CalDAVDates::class,
            CalDAVTasks::class,
        ...
        return $this->vaults()
            ->map(fn (Vault $vault) => $backends->map(fn (string $backend) => app($backend)->withUser($this->user)->withVault($vault)
            )
            )
            ->flatten(1);
    }
    ...
    private function getBackend($id): ?AbstractCalDAVBackend
    {
        return $this->getBackends()
            ->first(fn (IDAVBackend $backend) => $id === $backend->backendId());
    }
    ...
    public function getCalendarsForUser($principalUri)
    {
        ...
        return $backends
            ->map(fn (AbstractCalDAVBackend $backend) => $backend->getDescription())
            ->toArray();
    }
    ...
    {
        return optional($this->getBackend($calendarId))
            ->getChanges($calendarId, $syncToken) ?? [];
    }
    ...
    public function getCalendarObjects($calendarId)
    {
        ...
        if ($backend) {
            ...
            return $objs
                ->map(fn ($date) => $backend->prepareData($date))
                ->filter()
                ->toArray();
        }
    ...
    public function updateCalendarObject($calendarId, $objectUri, $calendarData): ?string
    {
        ...
        return optional($backend)
            ->updateOrCreateCalendarObject($calendarId, $objectUri, $calendarData);
    }
