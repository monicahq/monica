...
class AddBirthdayReminderToContacts extends Migration
{
    ...
    public function up()
    {
        Schema::table('contacts', function (Blueprint $table) {
            ...
            $table->foreign('birthday_reminder_id')->references('id')->on('reminders')->onDelete('cascade');
            $table->unsignedInteger('first_met_reminder_id')->nullable()->after('first_met_special_date_id');
            $table->foreign('first_met_reminder_id')->references('id')->on('reminders')->onDelete('cascade');
            $table->unsignedInteger('deceased_reminder_id')->nullable()->after('deceased_special_date_id');
            $table->foreign('deceased_reminder_id')->references('id')->on('reminders')->onDelete('cascade');
        });
    }
}
