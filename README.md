...
Before this assignment, there was no existing implementation for CSV contact imports in this system. I built a brand new CSV import flow from scratch that runs in a background queue. I designed it this way because doing huge CSV imports directly in the HTTP request is a bad idea—it can cause timeouts and memory crashes. So instead of doing it synchronously, the new queue-based system handles large files safely.
...
4. **Additional retry or idempotency protection**: Added strict business-logic idempotency. If a row contains an email or phone number that _already exists_ in the target vault, the background worker safely rejects that specific row instead of duplicating the data, without crashing the rest of the import.
