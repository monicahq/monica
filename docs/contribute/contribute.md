...
When adding new routes that inject Models, you need to either 
* use Laravel's route helper to inject the model (this is the preferred way)
* if for some reason you can't call the route helper instead of putting the ID in the URL, call `hashID()` on the model you are trying to inject this is to ensure that URLs are not leaking information. 

Also, if this is the first time that Model is being used in a route, ensure that it uses the `hasher` trait.
