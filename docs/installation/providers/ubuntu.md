...
1. Update `.env` to your specific needs
    ...
    - set the `APP_ENV` variable to `production`, `local` is only used for the development version. Beware: setting `APP_ENV` to `production` will force HTTPS. Skip this instruction if you're running Monica locally.
