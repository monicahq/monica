  ...
  <div>
    <div>
      ...
      <h3>
        ...
        <span v-if="tasks.length != 0" class="f6 pt2" :class="[ dirltr ? 'fr' : 'fl' ]">
          <a v-if="!editMode" class="pointer" cy-name="task-toggle-edit-mode" @click.prevent="editMode = true" href="">
            {{ $t('app.edit') }}
          </a>
          <a v-else class="pointer" cy-name="task-toggle-edit-mode" @click.prevent="editMode = false" href="">
            {{ $t('app.done') }}
          </a>
        </span>
      </h3>
    ...
    <div :class="[editMode ? 'bg-washed-yellow b--yellow ba pa2' : '']">
      ...
      <div v-if="tasks.length == 0 && !addMode" class="tc bg-near-white b--moon-gray pa3" cy-name="task-blank-state">
        ...
        <p>
          <a class="pointer" cy-name="add-task-button" @click.prevent="toggleAddMode" href="">
        ...
        <li v-for="(task, i) in inProgress(tasks)" :key="task.id" :cy-name="'task-item-' + task.id">
          ...
          <form v-show="task.edit" class="bg-near-white pa2 br2 mt3 mb3">
            ...
            <div class="lh-copy mt3">
              <a class="btn btn-primary" @click.prevent="update(task, true)" href="">
                {{ $t('app.update') }}
              </a>
              <a class="btn" @click.prevent="toggleEditMode(task)" href="">
                {{ $t('app.cancel') }}
              </a>
            </div>
          </form>
      ...
      <div v-if="!updateMode && !addMode && tasks.length != 0" class="bg-near-white pa2 br2 mt3 mb3">
        <a class="pointer" @click.prevent="toggleAddMode" href="">
          {{ $t('people.tasks_add_task') }}
        </a>
      </div>
      ...
      <div v-if="addMode" cy-name="task-add-view">
        <form class="bg-near-white pa2 br2 mt3 mb3">
          ...
          <div class="lh-copy mt3">
            <a class="btn btn-primary" cy-name="save-task-button" @click.prevent="store" href="">
              {{ $t('app.add') }}
            </a>
