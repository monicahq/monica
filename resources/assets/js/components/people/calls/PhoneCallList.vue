  ...
  <div>
    <div class="">
      <h3 class="mb2">
        ...
        <span class="fr relative" style="top: -7px;">
          <a v-if="displayLogCall == false" class="btn edit-information" @click.prevent="displayLogCall = true" href="">
            {{ $t('people.modal_call_title') }}
          </a>
          <a v-if="displayLogCall" class="btn edit-information" @click.prevent="displayLogCall = false" href="">
            {{ $t('app.cancel') }}
          </a>
        </span>
      </h3>
    ...
    <div v-if="!displayLogCall && calls.length == 0" class="w-100">
      <div class="bg-near-white tc pa3 br2 ba b--light-gray">
        ...
        <a class="pointer" @click.prevent="displayLogCall = true" href="">
          {{ $t('people.modal_call_title') }}
        </a>
      </div>
    </div>
      ...
      <div v-if="displayLogCall" class="ba br3 mb3 pa3 b--black-40">
        ...
        <div class="pt3">
          <div class="flex-ns justify-between">
            <div class="">
              <a class="btn btn-secondary tc w-auto-ns w-100 mb2 pb0-ns" @click.prevent="displayLogCall = false; resetFields()" href="">
                {{ $t('app.cancel') }}
              </a>
            </div>
            <div class="">
    ...
    <div v-for="call in calls" :key="call.id" class="ba br2 b--black-10 br--top w-100 mb2">
      ...
      <div v-show="editCallId == call.id" class="pa2">
        <div>
          ...
          <div class="">
            <div class="flex-ns justify-between">
              <div class="">
                <a class="btn btn-secondary tc w-auto-ns w-100 mb2 pb0-ns" @click.prevent="editCallId = 0" href="">
                  {{ $t('app.cancel') }}
                </a>
              </div>
              <div class="">
      ...
      <div class="pa2 cf bt b--black-10 br--bottom f7 lh-copy">
        ...
        <div :class="[ dirltr ? 'fl tr' : 'fr tl' ]" class="w-30">
          <a :class="[ dirltr ? 'mr2' : 'ml2' ]" class="pointer " @click.prevent="showEditBox(call)" href="">
            {{ $t('app.update') }}
          </a>
          <a v-show="destroyCallId != call.id" class="pointer" @click.prevent="showDestroyCall(call)" href="">
            {{ $t('app.delete') }}
          </a>
          <ul v-show="destroyCallId == call.id" class="di">
            <li class="di">
              <a class="pointer mr1" @click.prevent="destroyCallId = 0" href="">
                {{ $t('app.cancel') }}
              </a>
            </li>
            <li class="di">
              <a class="pointer red" @click.prevent="destroyCall(call)" href="">
