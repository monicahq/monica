    ...
    <div class="">
      <h3>
        ...
        <span v-show="reachLimit == 'false'" class="fr relative" style="top: -7px;">
          <a v-if="displayUploadZone == false && displayUploadError == false && displayUploadProgress == false" class="btn edit-information" @click.prevent="displayUploadZone = true" html="">
            {{ $t('people.document_list_cta') }}
          </a>
          <a v-if="displayUploadZone || displayUploadError || displayUploadProgress" class="btn edit-information" @click.prevent="displayUploadZone = false; displayUploadError = false; displayUploadProgress = false" href="">
            {{ $t('app.cancel') }}
          </a>
        </span>
      </h3>
      ...
      <div v-for="document in documents" :key="document.id" class="table-row">
        ...
        <div class="table-cell document-action relative">
          ...
          <ul v-if="modalToDisplay == document.id" class="absolute bg-white z-max pv1 document-action-menu">
            ...
            <li class="tc">
              <a class="pv2 pointer ph3 inline-flex items-center no-underline w-100 document-action-menu-item delete" @click.prevent="deleteDocument(document)" href="">
                {{ $t('app.delete') }}
              </a>
            </li>
          </ul>
