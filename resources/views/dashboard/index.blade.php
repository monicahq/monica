                    ...
                    <div class="col-xs-12 col-sm-9">
                        ...
                        <div class="dashboard-box dashboard-stat">
                            ...
                            <ul class="horizontal">
                                ...
                                <li>
                                    <span class="stat-number">{{ $user->currency->symbol }}{{ $debt_owed }}</span>
                                    <span class="stat-description">{{ trans('dashboard.statistics_deb_owed') }}</span>
                                </li>
                                <li>
                                    <span class="stat-number">{{ $user->currency->symbol }}{{ $debt_due }}</span>
                        ...
                        <span class="reminder-in-days">
                          {{ trans('dashboard.reminders_in_days', ['number' => $reminder->next_expected_date->diffInDays(Carbon\Carbon::now())]) }}
                            ({{ \App\Helpers\DateHelper::getShortDate($reminder->getNextExpectedDate()) }})
                                                    ...
                                                    <a href="/people/{{ $reminder->contact->id }}">{{ $reminder->contact->name }}</a>:
                                                    {{ $reminder->getTitle() }}
                                                </li>
                                            @endforeach
                                        </ul>
                                    ...
                                    @if ($tasks->count() != 0)
                                        <ul>
                                            @foreach ($tasks as $task)
                                                <li>
                                                    <a href="/people/{{ $task->contact_id }}">{{ $task->contact->name }}</a>:
                                                    {{ $task->getTitle() }}
                                                    {{ $task->getDescription() }}
                                                </li>
                                            @endforeach
                                    ...
                                    @if ($debts->count() != 0)
                                        <ul>
                                            @foreach ($debts as $debt)
                                                <li>
                                                    <a href="/people/{{ $debt->contact_id }}">{{ $debt->contact->name }}</a>:
